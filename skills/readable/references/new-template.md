...
7. **Sample content.** About three lorem chapters holding one exemplar of every block the template styles. `content/index.md` uses `mode: study` and also carries a `summary`, so both modes have something to show. Placeholders only: Lorem ipsum, `00` for numbers, `[Source, Year]` for citations. Keep the sample chapters short, and state the skill's "Keep it short" rule in `CONTENT.md` (the least text that carries each point; sections only where they help), so a filler who reads only the template still writes short.
